...
En Java hi ha **dues famílies** segons el tipus de dades que tractes: els **fluxos de bytes** per a informació binària (imatges, vídeos, executables) i els **fluxos de caràcters** per a **text Unicode** (fitxers `.txt`, `.csv`, `.xml`, `.html`, `.json`, etc.).
...
![alt text](Fitxers/streams1.webp)

---

> **Nota: (encoding):**
> L’**encoding** és la “traducció” **caràcters ⇄ bytes** (ex.: **UTF-8**).
> Per **text**, usarem `Reader/Writer` **indicant UTF-8** i per **binari**, `InputStream/OutputStream` (sense conversió).
...
* **Tanca sempre els fluxos.** Usa **try-with-resources**: és un `try ( … ) { … }` on cada recurs (classe que implementa `AutoCloseable`, com els fluxos d’`java.io`) es **tanca automàticament** en eixir del bloc, **encara que hi haja excepció**, i en **ordre invers** a la declaració. El `catch` és **opcional** (pots capturar o **propagar** amb `throws`).
  ...
  ```java
  // Exemple: llegir bytes d’un fitxer binari
...
> * Per a **text**, especifica l’**encoding** (p. ex. `"UTF-8"`).
> * Per a **binari**, treballa directament amb `InputStream`/`OutputStream`.
...
**Idea clau:** en Java I/O, quasi tots els “*streams* decoradors” **emboliquen** un altre stream per **afegir funcionalitats** sense heretar-ne el codi.
