...
*Què fa:* obri el fitxer, aplica buffer, i llix un `int` (4 bytes) del principi. Si ocorre un error d’E/S, el capturem i informem.

```java
try (java.io.FileInputStream fis = new java.io.FileInputStream("fitxer.bin");           // 1) font de bytes
     java.io.BufferedInputStream bis = new java.io.BufferedInputStream(fis);            // 2) buffer per millorar rendiment
     java.io.DataInputStream dis = new java.io.DataInputStream(bis)) {                   // 3) lectura de tipus primitives
...
*Què fa:* obri `in.txt` en mode text amb **UTF-8** (via `InputStreamReader`) i copia línies a `out.txt` amb `BufferedWriter`.
*(Sense `java.nio`; només `java.io`, per això l’encoding s’indica com a String `"UTF-8"`.)*

```java
try (java.io.BufferedReader br = new java.io.BufferedReader(
         new java.io.InputStreamReader(new java.io.FileInputStream("in.txt"), "UTF-8"));  // bytes -> caràcters (UTF-8)
     java.io.BufferedWriter bw = new java.io.BufferedWriter(
         new java.io.OutputStreamWriter(new java.io.FileOutputStream("out.txt"), "UTF-8"))) { // caràcters -> bytes (UTF-8)
...
**Exemple C3 — Sense `catch`, propagant amb `throws` (es veu el `throws`)**
*Què fa:* crea un CSV simple. Qualsevol `IOException` **no** es captura ací; es **propaga** al cridador gràcies al `throws java.io.IOException` en la signatura.

```java
// Propaguem l'excepció: qui cride exportaCSV() haurà de capturar o tornar a propagar
void exportaCSV() throws java.io.IOException {
    try (java.io.BufferedWriter bw = new java.io.BufferedWriter(
             new java.io.OutputStreamWriter(new java.io.FileOutputStream("dades.csv"), "UTF-8"))) {
