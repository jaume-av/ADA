...
- **@JsonProperty**: Ens permet personalitzar el nom de les propietats en la sortida JSON, indicant com s'haurà de serialitzar o deserialitzar una propietat específica.
...
- **@JsonInclude**: Controla quines propietats s'inclouran en la sortida JSON quan algunes d'elles tenen valors nuls o buits.
...
- **@JsonIgnore**: Exclou propietats de la serialització i deserialització, es a dir, la propietat marcada com a `@JsonIgnore` no es tindrà en compte en el procés de convertir l'objecte a JSON ni en el procés de llegir un JSON i convertir-lo en un objecte Java.
...
- **@JsonFormat**: Permet definir com el format dels atributso propietats d'un objecte Java quan es converteixen a JSON i viceversa. S'utilitza per ajustar el format de dates i números, assegurant que les dades es mostren amb el format desitjat.n què oes mostraran a la sortida JSON.
...
- **@JsonCreator**: Etiqueta utilitzada per personalitzar la construcció d'objectes a partir de JSON. S’aplica a constructors i mètodes de fàbrica (“factory method”) que es fan servir per a crear instàncies d'una classe a partir de les dades contingudes en un objecte JSON.
...
- **@JsonValue**: Indica quin mètode o propietat s'ha d'utilitzar per a la serialització de l'objecte. Quan es converteix un objecte a JSON, el valor retornat pel mètode anotat amb `@JsonValue` serà el que es mostri en el JSON.
...
- **@JsonSetter**: Indica quin mètode s'ha d'utilitzar per a la deserialització.
